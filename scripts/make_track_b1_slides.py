...
def heading(slide, title, kicker=None):
    ...
    if kicker:
        _text(
            box(slide, MARGIN, top, BODY_W, Inches(0.32)),
            [kicker.upper()],
            size=12,
            color=PURPLE_MID,
            bold=True,
    ...
    _text(
        box(slide, MARGIN, top + Inches(0.18), BODY_W, title_h + Inches(0.1)),
        [title],
        size=TITLE_PT,
        color=PURPLE_DEEP,
        bold=True,
...
def stat_panel(slide, left, top, width, value, label, height=Inches(1.45)):
    ...
    _text(
        box(slide, left + Inches(0.2), top + Inches(0.16), width - Inches(0.4), Inches(0.6)),
        [value],
        size=30,
        color=PURPLE_DEEP,
        bold=True,
    ...
    _text(
        box(slide, left + Inches(0.2), top + Inches(0.83), width - Inches(0.4), Inches(0.5)),
        [label],
        size=12,
        color=MUTED,
        space_after=0,
...
def build(output: Path, figures: Path) -> None:
    ...
    _text(
        box(s, MARGIN, Inches(2.7), Inches(11.2), Inches(2.1)),
    ...
    _text(
        box(s, MARGIN, Inches(4.35), Inches(10.6), Inches(1.0)),
        [
            "Actor-conditioned video transformers, and why direction of transfer "
            "does not generalise across recording days"
        ],
        size=20,
        color=RGBColor(0xD6, 0xC7, 0xEA),
        space_after=0,
        line=1.3,
    )
    _text(
        box(s, MARGIN, Inches(5.9), Inches(10.6), Inches(0.9)),
        [
            "Track B1  ·  Summer School 2026 Computer Vision Case",
            "42 annotated recordings  ·  259 events  ·  VideoMAE-Base",
        ],
        size=14,
        color=RGBColor(0xB6, 0xA2, 0xD4),
        space_after=5,
    )
    notes(
        s,
        "Talk is ~20 minutes. Two methodological findings and one negative result. "
    ...
    for i, (value, label) in enumerate(
        [
            ("259", "annotated events"),
            ("170 / 89", "pickup / putdown"),
            ("42", "recordings"),
            ("5", "recording days"),
        ]
    ):
    ...
    notes(
        s,
        "Frame the asymmetry early: a false pickup is a charge for something the "
    ...
    _text(
        box(s, Inches(8.6), top + Inches(0.3), Inches(3.6), Inches(2.5)),
        [
            ("Consequence", True),
    ...
    notes(
        s,
        "This slide sets up the negative result 12 slides later. Direction is the "
    ...
    bullets(
        ...
        [
            ...
            (
                ...
                "temporal extent and the spatial region in one artefact.",
                True,
            ),
    ...
    _text(
        box(s, Inches(8.95), top + Inches(0.35), Inches(3.3), Inches(2.1)),
        [
            ("Median event", True),
            "0.90 seconds",
            ("Median annotation box", True),
            "214 × 249 px in 4K",
        ],
        size=15,
        color=PURPLE_DEEP,
    ...
    notes(
        s,
        "The box is the key asset: it removes the need for a pose pipeline, but it "
    ...
    for i, (title_txt, sub) in enumerate(stages):
        ...
        _text(
            box(s, left + Inches(0.1), top + Inches(0.6), width - Inches(0.2), Inches(0.8)),
            [title_txt],
            size=14,
            color=PURPLE_DEEP,
            bold=True,
        ...
        _text(
            box(s, left + Inches(0.1), top + Inches(1.38), width - Inches(0.2), Inches(0.7)),
            [sub],
            size=11,
            color=MUTED,
            space_after=0,
    ...
    bullets(
        ...
        [
            ...
            (
                ...
                "window cropped around another — 273 overlapping actor pairs in this corpus.",
                True,
            ),
    ...
    _text(
        box(s, Inches(8.95), top + Inches(0.32), Inches(3.3), Inches(1.9)),
        [
    ...
    notes(
        s,
        "Three actors, overlapping in time, different event types — the acceptance "
    ...
    fill(s, WHITE)
    top = heading(
        s, "Conditioning available only for positives leaks the label", "Design decision 1"
    )
    bullets(
        s,
        [
            ...
            (
                ...
                "classes become separable on crop size alone.",
                True,
            ),
            ...
            (
                ...
                "negatives from a candidate share geometry.",
                True,
            ),
    ...
    notes(
        s,
        "Generalises beyond this project: any region-conditioned formulation where "
    ...
    table(
        ...
        [
            ["", "Naive", "After", "Effect"],
            [
                "Decode a window (4K, seek per frame)",
                "20.6 s / 16 frames",
                "sequential, cached once",
                "~17 h → ~9 min per epoch",
            ],
            [
                "Frozen backbone recomputed each epoch",
                "86.2 M params",
    ...
    bullets(
        ...
        [
            ...
            (
                ...
                "threshold sweep and honest early stopping affordable.",
                True,
            ),
    ...
    for i, (name, body) in enumerate(
        ...
        _text(
            box(s, left + Inches(0.3), top + Inches(0.28), Inches(5.0), Inches(0.5)),
            [name],
            size=18,
            color=PURPLE_DEEP,
            bold=True,
        ...
        _text(
            box(s, left + Inches(0.3), top + Inches(0.92), Inches(5.0), Inches(2.0)),
            body.split("\n\n"),
            size=14,
            color=INK,
            space_after=10,
    ...
    s = blank(prs)
    fill(s, WHITE)
    top = heading(s, "A window wider than the event caps tIoU", "Finding 1")
    bullets(
        s,
        [
    ...
    notes(
        s,
        "Arithmetic, not a modelling limitation. Worth computing d/w in advance "
        "whenever sliding-window detection is scored with tIoU.",
    )

    figure_slide(
        prs,
        "Decoder geometry dominated the ablation",
    ...
    notes(
        s,
        "Honest note: our first fine-tuning run was misconfigured and would have "
        "supported the wrong conclusion.",
    )

    figure_slide(
        prs,
        "Validation holds up; the held-out day does not",
    ...
    figure_slide(
        prs,
        "No threshold recovers the class",
        figures / "fig3_putdown_probability_shift.png",
        [
            ("Predicted putdown probability on windows whose ground truth is putdown.", True),
    ...
    _text(
        box(s, MARGIN + Inches(0.4), Inches(5.75), BODY_W - Inches(0.8), Inches(0.8)),
        [
    ...
    bullets(
        ...
        [
            (
                ...
                "is not deployable as it stands.",
                True,
            ),
    ...
    notes(
        s,
        "State the oracle-conditioning caveat plainly. An audience that spots it "
    ...
    for i, (num, title_txt, body) in enumerate(
        ...
        _text(
            box(s, left + Inches(0.28), top + Inches(0.24), Inches(1.0), Inches(0.6)),
            [num],
            size=30,
            color=PURPLE_MID,
            bold=True,
        ...
        _text(
            box(s, left + Inches(0.28), top + Inches(0.88), Inches(3.1), Inches(0.6)),
            [title_txt],
            size=16,
            color=PURPLE_DEEP,
            bold=True,
        ...
        _text(
            box(s, left + Inches(0.28), top + Inches(1.6), Inches(3.1), Inches(1.4)),
            [body],
            size=13,
            color=INK,
            space_after=0,
    ...
    notes(
        s,
        "Ordered by expected value. Cross-validation first — without it no later "
    ...
    _text(
        box(s, MARGIN, Inches(1.35), Inches(11.2), Inches(0.9)),
        ["Conclusions"],
        size=36,
        color=WHITE,
        bold=True,
    ...
    _text(
        box(s, MARGIN, Inches(2.4), Inches(11.2), Inches(3.6)),
        [
        ...
        size=17,
        color=RGBColor(0xE4, 0xDA, 0xF2),
        space_after=14,
        line=1.3,
    )
    _text(
        box(s, MARGIN, Inches(6.25), Inches(11.2), Inches(0.6)),
        [
    ...
    prs.save(str(output))
    print(
        f"wrote {output}  ({len(prs.slides.__iter__.__self__._sldIdLst)} slides, "
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path, default=REPO_ROOT / ".local/paper/track_b1_presentation.pptx"
    )
