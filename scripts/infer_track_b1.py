...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--clips", nargs="+", default=None, help="restrict to these clip ids (smoke runs)"
    )
    parser.add_argument(
        "--config",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if checkpoint_info.get("input_mode") not in (None, mode):
        logger.warning(
            "checkpoint trained in %s mode, evaluated on %s inputs",
            checkpoint_info["input_mode"],
            mode,
        )
    model.eval()
    ...
    with torch.inference_mode():
        for step, batch in enumerate(loader, start=1):
            probabilities.append(
                torch.softmax(model(batch["pixel_values"].to(device)), dim=-1).cpu().numpy()
            )
            if step % 25 == 0 or step == len(loader):
                logger.info("  %d/%d batches", step, len(loader))
    probs = np.concatenate(probabilities)
    scores = dataset.manifest[
        ["clip_id", "candidate_id", "actor_id", "window_start_s", "window_end_s"]
    ].copy()
    ...
    label = (
        "ANNOTATION-CONDITIONED (GT candidates/crops)"
        if mode == "annotation"
        else "DEPLOYMENT-INPUT (pose candidates/crops)"
    )
    print(
    ...
    for tiou in args.tiou:
        ...
        print(
            f"  tIoU {tiou}: P={row['precision']:.3f} R={row['recall']:.3f} F1={row['f1']:.3f} "
