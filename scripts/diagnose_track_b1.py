...
def select_review(df, limit=12):
    ...
    groups = {
        ...
        "event_confusion": df[(df.true_id != 0) & (df.pred_id != 0) & (df.true_id != df.pred_id)],
    }
    groups = {
        name: list(rows.sort_values("confidence", ascending=False).index)
...
def legacy_dataset(args):
    ...
    ignores = (
        pd.read_parquet(ignore_path)
        if ignore_path.exists()
        else pd.DataFrame(columns=["clip_id", "t_start", "t_end", "reason"])
    )
    ...
    manifest = build_window_manifest(
        ...
        events_df=pd.read_csv(data / "events_human.csv"),
    ...
    frame_cache = (
        Path(cfg.get("frame_cache_dir", args.frame_cache_dir))
        if cfg.get("cache_frames", True)
        else None
    )
    dataset = TrackB1Dataset(
        ...
        args.video_dir,
        data / "pose_tracks",
        load_shelf_regions(args.shelves),
        window,
        clips_df=clips,
...
def cvat_event_lookup(evidence_dir, legacy_events=None):
    ...
    """
    events = (
        evidence_dir.table("events_pose_identity")
        if evidence_dir.input_mode == "deployment"
...
def add_evidence(result, window, tracks_dir, shelf_regions, evidence_dir, lookup):
    ...
    cvat_dir = evidence_dir.path / (
        "cvat_tracks" if evidence_dir.input_mode == "deployment" else "actor_tracks"
    )
    crop_tracks, cvat_tracks = {}, {}
    columns = {
        k: []
        for k in (
            "matched_cvat_event_id",
            "event_box_in_crop",
            "sampled_frames_in_event",
            "crop_is_full_frame",
        )
    }
    for _, row in result.iterrows():
        ...
        crop = compute_actor_crop_box(
            actor, shelf, *crop_span(row, window), window.crop_margin, frame_size
        )
...
def main():
    ...
    parser.add_argument(
        "--frame-cache-dir", type=Path, default=Path(".local/track_b1_frame_cache")
    )
    ...
    for idx, row in selected.iterrows():
        path = (
            review_dir / f"{int(row.dataset_index):05d}_{row.true_label}_to_{row.pred_label}.mp4"
        )
